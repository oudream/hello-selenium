...
class AppDynamicsJob(unittest.TestCase):
    ...
    def test_app_dynamics_job(self):
        ...
        driver.get("https://192.168.5.29/web/goods/4212?goodsId=6999")
        driver.execute_script("location.reload(true);")
        now = time.perf_counter_ns()
        print('-------------------- begin : ' + str(datetime.datetime.now()))
        r = self.wait_until_images_loaded(driver)
        print('-------------------- cost : ' + str(r) + ' --- ' + str((time.perf_counter_ns()-now) // 1000000))
        ele = driver.find_element_by_css_selector('#addCartBtn')
        print('-------------------- cost : ' + str((time.perf_counter_ns()-now) // 1000000))
        print('------------------------1----')
        print(ele)
        print('------------------------2----')
        print(ele.text)
        print('-------------------- cost : ' + str((time.perf_counter_ns()-now) // 1000000))
        print('------------------------2----')
        print(ele.get_property('innerHTML'))
        print('-------------------- cost : ' + str((time.perf_counter_ns()-now) // 1000000))
        print('------------------------3----')
        ...
        print('------------------------5----')
        print('------------------------3----')
        print(ele.get_property('outerText'))
        print('-------------------- cost : ' + str((time.perf_counter_ns()-now) // 1000000))
        print('------------------------4----')
        print(ele.get_property('outerHTML'))
        print('-------------------- cost : ' + str((time.perf_counter_ns()-now) // 1000000))
        print('------------------------5----')
